  ...
  fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?i=${searchQuery}`)
    ...
    .then((data) => {
      ...
      if (data.meals) {
        ...
        searchResultsContent = renderMeals(data, searchResultsContent);
        ...
        generateErrorMessage(searchQuery);
      }
      searchTitle.classList.remove("hidden");
    });
}

function renderMeals(data, searchResultsContent) {
...
function generateErrorMessage(searchQuery) {
  const errorMessages = [
